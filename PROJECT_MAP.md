...
- **Live GenAI evidence gap (Phase 2, found 2026-09-25 14:09, closed 2026-09-25 15:05)**: the first live run exposed that (a) `gemini-2.5-flash` is dead for new keys (default kept; `GEMINI_MODEL` override proven with `gemini-3.5-flash-lite`), (b) the plan template never contained the schema (fixed with `response_json_schema` + schema passthrough; 27 tests still green), (c) free-tier quota (5/min, 20/day) bounds full 10-role runs (harness paces + honors `retryDelay`). Evidence: 1 Gemini-backed plan with 0 retries, 1 honest assembler fallback, unedited raw response — all in `reports/d4_live_genai_run.json`. Rerun after quota reset for remaining roles; `max_output_tokens` truncation hypothesis still unverified by design.
